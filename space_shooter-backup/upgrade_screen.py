...
class UpgradeScreen:
    ...
    def run(self):
        while self.running:
            ...
            draw_text(self.screen, f"Points: {self.upgrade_data['points']}", settings.SCREEN_WIDTH // 2, 50, self.font, settings.WHITE)
    ...
    def draw_upgrade_options(self):
        y_start = 150
        draw_text(self.screen, "アップグレード:", settings.SCREEN_WIDTH // 2 - 150, y_start - 30, self.font, settings.YELLOW)
        upgrade_names = {
            'attack': '攻撃力',
            'fire_rate': '連射速度',
            'speed': '移動速度',
        ...
        for i, (key, cost) in enumerate(self.upgrade_costs.items()):
            ...
            draw_text(self.screen, f"{display_name} (Lv: {level})", settings.SCREEN_WIDTH // 2 - 150, y_start + i * 60, self.small_font, settings.WHITE)
            draw_text(self.screen, f"コスト: {cost * level}", settings.SCREEN_WIDTH // 2 - 150, y_start + i * 60 + 20, self.small_font, settings.GRAY)

    def draw_weapon_options(self):
        y_start = 150
        x_start = 50
        draw_text(self.screen, "武器:", x_start + 75, y_start - 30, self.font, settings.YELLOW)
        weapon_names = {
            'normal': '通常弾',
            'wide_shot': 'ワイドショット',
            'laser_weapon': 'レーザー'
        ...
        for i, weapon_type in enumerate(settings.WEAPON_TYPES):
            ...
            draw_text(self.screen, display_name, x_start + 75, y_start + i * 60, self.small_font, text_color)
            if not is_unlocked and weapon_type != "normal":
                draw_text(self.screen, f"コスト: {cost}", x_start + 75, y_start + i * 60 + 20, self.small_font, settings.GRAY)
    ...
    def draw_buttons(self):
        ...
        for weapon_type, rect in self.weapon_buttons.items():
            ...
            if is_current: button_color = settings.BLUE # 選択中の武器は青
