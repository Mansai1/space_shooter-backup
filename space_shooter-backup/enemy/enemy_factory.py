...
class EnemyFactory:
    ...
    @classmethod
    def create_enemy(cls, enemy_type, x, y, player, level_config=None, **kwargs):
        """指定されたタイプの敵を生成"""
        # レベル設定から倍率を取得
        ...
        return enemy_class(x, y, player, level_multipliers=level_multipliers, **kwargs)
    
    @classmethod
    def create_random_enemy(cls, x, y, player, level_config=None):
        """レベル設定に基づいてランダムな敵を生成"""
        if level_config:
        ...
        return cls.create_enemy(enemy_type, x, y, player, level_config)
    
    @classmethod
    def create_enemy_wave(cls, wave_type, start_x, start_y, player, level_config=None):
        """レベル設定に基づいて敵の編隊を生成"""
        enemies = []
        ...
        if wave_type == "basic_line":
            ...
            for i in range(5):
                x = start_x + (i - 2) * 60
                if 0 <= x <= SCREEN_WIDTH:
                    enemy = cls.create_enemy('basic', x, start_y, player, level_config)
            ...
            for i in range(3):
                x = start_x + (i - 1) * 80
                if 0 <= x <= SCREEN_WIDTH:
                    enemy = cls.create_enemy('fast', x, start_y - i * 30, player, level_config)
                    enemies.append(enemy)
            ...
            if level_config and 'zigzag' in level_config.get('enemy_types', []):
                for i in range(4):
                    x = start_x + (i - 1.5) * 70
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('zigzag', x, start_y - i * 20, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config:
                ...
                for i in range(6):
                    ...
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy_type = random.choice(available_types)
                        enemy = cls.create_enemy(enemy_type, x, start_y - (i % 2) * 25, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config and 'tank' in level_config.get('enemy_types', []):
                for i in range(3):
                    x = start_x + (i - 1) * 100
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('tank', x, start_y, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config and 'shield' in level_config.get('enemy_types', []):
                for i in range(4):
                    x = start_x + (i - 1.5) * 60
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('shield', x, start_y, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config and 'stopper' in level_config.get('enemy_types', []):
                ...
                for i, x in enumerate(positions):
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('stopper', x, start_y - i * 40, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config and 'sniper' in level_config.get('enemy_types', []):
                ...
                for x in positions:
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('sniper', x, start_y, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config and 'kamikaze' in level_config.get('enemy_types', []):
                for i in range(5):
                    x = start_x + (i - 2) * 40
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('kamikaze', x, start_y - i * 15, player, level_config)
                        enemies.append(enemy)
            ...
            if (level_config and 'tank' in level_config.get('enemy_types', []) 
                ...
                # 中央にタンク
                tank = cls.create_enemy('tank', start_x, start_y, player, level_config)
                enemies.append(tank)
                ...
                for x, y in shield_positions:
                    if 0 <= x <= SCREEN_WIDTH:
                        enemy = cls.create_enemy('shield', x, y, player, level_config)
                        enemies.append(enemy)
            ...
            if level_config:
                ...
                right_x = min(SCREEN_WIDTH - 50, start_x + 200)
                
                for i in range(3):
                    enemy_type = random.choice(available_types)
                    
                    # 左側編隊
                    enemy_left = cls.create_enemy(enemy_type, left_x, start_y - i * 40, player, level_config)
                    enemies.append(enemy_left)
                    
                    # 右側編隊
                    enemy_type = random.choice(available_types)
                    enemy_right = cls.create_enemy(enemy_type, right_x, start_y - i * 40, player, level_config)
